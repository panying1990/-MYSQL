...
-- 拆分销售区域全路径
CREATE DEFINER=`root`@`localhost` FUNCTION `getarea`(sale_allpath varchar(255),area_level int) RETURNS varchar(150) CHARSET utf8mb4
BEGIN
  ...
  elseif (area_level = 3) then 
	...
	set out_area = substring_index(substring_index(sale_allpath,'/',area_level),'/',-1);
	elseif (area_level = 6) then 
	set out_area = substring_index(substring_index(sale_allpath,'/',area_level),'/',-1);
	elseif (area_level = -1 and right(substring_index(sale_allpath,'/',area_level),1)=':') then 
	set out_area =  substring_index(substring_index(sale_allpath,'/',-2),'/',1);
	elseif (area_level = -1 and right(substring_index(sale_allpath,'/',area_level),1)<>':') then 
	set out_area =  substring_index(sale_allpath,'/',-1);
	else set out_area = '';
	end if;
	RETURN out_area;
END
